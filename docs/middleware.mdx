...
id: middleware
title: Middleware
description: Add type-safe middleware to api-def APIs and endpoints for authentication, request changes, events and shared behaviour.
---
...
| Name           | Usage |
| ---            | --- |
| `beforeSend`   | Runs before validation and the backend request. |
| `beforeRequest` | Runs after request mutation and validation, immediately before the backend request. |
| `success`      | Runs once when the request succeeds. |
| `attemptError` | Runs for each failed backend attempt and may respond or request a retry. |
| `error`        | Runs once when the overall request is about to reject, including middleware and validation failures. |
| `finally`      | Runs once after every successful or failed request. |

`error` and `finally` are observers; their return values are ignored. Use `attemptError` when middleware needs to recover from a failed attempt or trigger a retry.
...
Applicable in the `beforeSend`, `beforeRequest`, and `attemptError` events.
...
Applicable in the `attemptError` event.
...
Use `finally` to release resources registered for a request. Inspect `context.response` and `context.error` to determine the outcome:

```typescript
const cleanupMiddleware: RequestMiddleware = {
  finally: (context) => {
    pendingRequests.delete(context.id);
  },
};
```
